...
public class Frame {
    ...
    public Frame() {
        ...
        if (!GLFW.glfwInit()) {
            throw new IllegalStateException("Impossible d'initialiser OpenGL: error 0");
        }
        ...
        if (window == 0) {
            throw new RuntimeException("Error: impossible d'initialiser la fenêtre: error 1");
        }
        ...
        int loadingTexture = loadTexture.getTexture("asset/mojang.png");

        renderLoadingScreen(loadingTexture, window);
    ...
    public void renderLoadingScreen(int texture, long window) {
    ...
    public void moove(long window, Camera camera) {
        if (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_W) == GLFW.GLFW_PRESS) camera.moveForward(0.8f);
        if (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_S) == GLFW.GLFW_PRESS) camera.moveForward(-0.8f);
        if (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_A) == GLFW.GLFW_PRESS) camera.moveRight(-0.8f);
        if (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_D) == GLFW.GLFW_PRESS) camera.moveRight(0.8f);
        if (GLFW.glfwGetKey(window, GLFW.GLFW_KEY_ESCAPE) == GLFW.GLFW_PRESS) System.exit(0);
    }
